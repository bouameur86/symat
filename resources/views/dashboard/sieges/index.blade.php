                        ...
                        <table class="table table-hover">
                            ...
                            @foreach ($postes as $index=>$poste)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $poste->name }}</td>
                                    <td>{{ $poste->products->count() }}</td>
                                    <td><a href="{{ route('dashboard.products.index', ['poste_id' => $poste->id]) }}" class="btn btn-info btn-sm">@lang('site.related_products')</a></td>
                                    <td>
                                        @if (auth()->user()->hasPermission('update_postes'))
                                            <a href="{{ route('dashboard.postes.edit', $poste->id) }}" class="btn btn-info btn-sm"><i class="fa fa-edit"></i> @lang('site.edit')</a>
                                        @else
