...
@section('content')
        ...
        <section class="content">
            ...
            <div class="box box-primary">
                ...
                <div class="box-body">
                    ...
                    <form action="{{ route('dashboard.anomalieexps.update', $anomalieexp->id) }}" method="post">

                        {{ csrf_field() }}
                        {{ method_field('put') }}
                       <!-- 1 numero message -->
                       <div class="form-group"> 
                        <label>Numero de message</label>
                        <input type="text" name="numes" class="form-control"  value="{{ $anomalieexp->numes }}" placeholder="N° BE / N°Poste ......  Exemple : 645/57 ">
                    </div>
                    <!-- 2 Date de l'anomalie -->
                    <div class="form-group"> 
                        <label>Date de l'anomalie</label>
                        <input type="date" name="dateanom" class="form-control"  value="{{ $anomalieexp->dateanom}}" >
                    </div>
                    <!-- 3 poste -->
                        ...
                        <label>Poste</label> 
                            <select  id="poste" class="form-control" name="poste_id">
                                ...
                                @foreach($postes as $poste)
                                    <option value="{{ $poste->id }}" {{ $anomalieexp->poste_id == $poste->id ? 'selected' : '' }}>{{ $poste->code }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="form-group">
                        <label for="equipement" >Choisir un equipement</label>
                            <select id="equipement" class="form-control" name="equipement" value="{{ $anomalieexp->equipement }}" >
                            <option>{{ $anomalieexp->equipement}}</option>
                            <option>TR 1</option>
                            <option>TR 2</option>
                            <option>TR 3</option>
                            <option>TR 4</option>
                    ...
                    <!--  5 Spécefie l'equipement -->
                            <div class="form-group"> 
                                ...
                                <input type="text" name="spequip" class="form-control"  value={{ $anomalieexp->spequip }}>
                            </div>
                     <!--  6 niveau de tension-->
                    <div class="form-group">
                        <label>Niveau de tension</label>
                            <select id="nivU" class="form-control" name="nivU" value="{{ old('nivU') }}" >
                                <option>{{ $anomalieexp->nivU }}</option>
            ...
            <div class="form-group">
                ...
                <select id="typeanom" class="form-control" name="typeanom" value="{{ old('typeanom') }}">
                        <option disabled selected>{{ $anomalieexp->typeanom }}</option>
                    ...
                    <select id="anomalie" class="form-control" name="anomalie" value= "{{ old('anomalie') }}">
                        <option>{{ $anomalieexp->anomalie }}</option>
                        <!-- SF6 -->
                ...
                <!--   9 Description   -->
                    <div class="form-group"> 
                        <label>Déscription de l'anomalie</label>
                        <textarea type="text" id="descranom" name="descranom" class="form-control"  rows="4"  value="{{ old('descranom') }}" >{{ $anomalieexp->descranom }}</textarea>
                    </div>
                    
                        <div class="form-group">
                            <label>Niveau de tension</label>
                                <select class="form-control" name="leon" value="{{ old('leon') }}" >
                                    <option disabled selected>{{ $anomalieexp->leon }}</option>
                                    <option>Oui</option>
                                    <option>Non</option>
    ...
    <script type="text/javascript">
     ...
     $("#anomalie").select2({
         placeholder: "Select anomalie",
         allowClear: true
     });
    $("#nivU").select2({
            placeholder: "Select (U) ",
            allowClear: true
        });
